...
import {setTaskName, setIsDone, setDescription} from "@/store/taskSlice";
export const TaskItem = (props: {index: number}) => {
    ...
    <div>
      <button className="EditButton" onClick={updateTask}>Submit</button>
        ...
        {tasksList[props.index].user === authState.user?.id ? <button onClick={() => setEdit(!edit)}>Edit</button> : null}
